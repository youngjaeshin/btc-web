...
const quizQuestions = [
  {
    ...
    options: [
      "새 비트코인을 채굴하여 보상을 받는다",
      "모든 트랜잭션과 블록을 독립적으로 검증하고 합의 규칙을 집행한다",
      "라이트닝 채널을 개설하여 소액결제를 처리한다",
      "거래소에서 비트코인을 매매한다",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "채굴자들이 사용하는 특수 하드웨어 장치",
      "블록체인에 영구 저장된 트랜잭션 목록",
  ...
  },
  {
    ...
    options: [
      "채굴자가 이미 확인된 트랜잭션을 되돌리는 기능",
      "더 높은 수수료를 붙인 새 트랜잭션으로 멤풀의 미확인 트랜잭션을 교체하는 기능",
      "여러 UTXO를 하나로 합치는 기능",
      "라이트닝 채널에서 잔액을 조정하는 기능",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "블록 크기를 8MB로 영구 확장하기로 합의했다",
      "채굴자들이 프로토콜 변경의 최종 결정권을 가짐이 확인되었다",
      "SegWit(Segregated Witness) 소프트포크로 효율성을 높이고 풀노드의 중요성이 재확인되었다",
      "비트코인 재단이 개발 방향을 통제하기로 결정했다",
    ],
    answer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      "노드가 많을수록 비트코인 채굴 속도가 빨라진다",
      "노드가 많을수록 거래 수수료가 낮아진다",
      "더 많은 독립적 검증자가 합의 규칙을 집행하므로 단일 기관의 프로토콜 변경이 어려워진다",
      "노드가 많을수록 블록 생성 시간이 단축된다",
    ],
    answer: 2,
    explanation:
  ...
  },
  {
    ...
    options: [
      "비트코인 가격이 달러로 표시되기 때문에 sat 단위가 편리하다",
      "블록 공간이 가중 바이트(vByte) 단위로 측정되므로, 면적당 수수료율로 공정하게 비교하기 위해서이다",
      "채굴자들이 sat/vByte 기준으로만 세금 신고를 하기 때문이다",
      "라이트닝 네트워크 수수료와 단위를 통일하기 위해서이다",
    ],
    answer: 1,
    explanation:
  ...
  return (
    <div>
      ...
      <InfoBox type="definition" title="풀노드(Full Node)란">
        비트코인 블록체인 전체를 독립적으로 검증하는 소프트웨어 노드. Bitcoin Core를 실행하는
        누구나 풀노드가 될 수 있습니다. 현재 전 세계에 ~15,000개 이상의 공개 풀노드가 운영 중입니다.
      </InfoBox>
      ...
      <InfoBox type="warning" title="수수료는 채굴자 수입의 미래">
        반감기를 거듭할수록 블록 보조금이 줄어듭니다. 장기적으로 채굴자 수입은 수수료에 의존해야
        합니다. 건전한 수수료 시장이 형성되어야 비트코인 보안 예산이 유지됩니다.
        일부 Bitcoiner들은 블록 공간의 희소성 유지가 장기 보안의 핵심이라고 봅니다.
      </InfoBox>
      ...
      <InfoBox type="warning" title="블록 크기 전쟁의 교훈 (2015–2017)">
        ...
        <br /><br />
        <strong>핵심 교훈:</strong> 채굴자도, 기업도, 개발자도 프로토콜 변경을 강제할 수 없습니다.
        최종 결정권은 경제적 노드(풀노드를 운영하는 사용자)에게 있습니다.
      </InfoBox>
      ...
      <InfoBox type="tip" title="지금 풀노드를 실행해보세요">
        ...
        Umbrel, Start9, RaspiBlitz 등 사용자 친화적 솔루션이 있습니다.
