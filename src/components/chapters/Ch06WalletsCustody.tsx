...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      "비트코인은 계좌 잔액처럼 하나의 숫자로 관리된다",
      "UTXO는 이전 거래에서 받은 미사용 출력으로, 새 거래의 입력으로 사용된다",
      "UTXO는 거래소에서만 사용하는 개념이다",
      "UTXO를 쪼개서 일부만 전송할 수 있다",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "주소가 공개되어도 거래 내역을 볼 수 없기 때문에",
      "주소에서 공개키나 개인키를 역산하는 것이 수학적으로 불가능하기 때문에",
      "비트코인 네트워크가 주소를 암호화하기 때문에",
      "주소는 매 거래마다 자동으로 바뀌기 때문에",
    ],
    answer: 1,
    explanation:
