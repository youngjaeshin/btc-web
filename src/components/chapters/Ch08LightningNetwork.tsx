...
const quizQuestions = [
  {
    ...
    options: [
      "비트코인 블록체인의 보안이 불충분하기 때문에",
      "비트코인 온체인은 처리량이 ~7 TPS로 제한되어 있어 소액·즉시 결제에 비효율적이기 때문에",
      "라이트닝 네트워크는 새로운 암호화폐를 발행하기 위해서",
      "채굴자들이 더 많은 수수료를 받기 위해 개발했기 때문에",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "채널의 존재를 채굴자에게 알리기 위해",
      "2-of-2 다중서명 주소에 자금을 잠그는 펀딩 트랜잭션이 필요하기 때문에",
      "라이트닝 네트워크 수수료를 지불하기 위해",
      "채널 파트너의 신원을 블록체인에 등록하기 위해",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "트랜잭션에 타임스탬프를 기록하여 이중 지불을 방지한다",
      "비밀값(프리이미지) R의 해시 H를 아는 사람만 자금을 청구할 수 있도록 조건을 설정한다",
      "채굴자가 블록을 생성할 때 사용하는 작업증명 메커니즘이다",
      "노드 간 통신을 암호화하는 TLS 프로토콜이다",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "결제 속도를 인위적으로 느리게 하기 위해",
      "중간 노드가 자금을 가로채거나 실패한 결제를 방치할 경우 환불 경로를 보장하기 위해",
      "비트코인 블록 생성 간격을 10분으로 고정하기 위해",
      "채널 용량을 자동으로 재조정하기 위해",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "한쪽이 최신 커밋먼트 트랜잭션을 일방적으로 브로드캐스트한다",
      "두 참여자가 합의하여 최신 잔액을 반영한 종료 트랜잭션에 함께 서명하고 즉시 자금을 받는다",
      "분쟁 해결을 위해 법원에 중재를 신청한다",
      "채굴자가 강제로 채널을 종료한다",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "비트코인 프로토콜이 초당 트랜잭션 스트리밍을 기본 지원하기 때문에",
      "채널 내 결제는 온체인 수수료나 블록 확인이 필요 없어 초당 수백~수천 sat 단위 결제가 가능하기 때문에",
      "라이트닝 노드가 자동으로 BTC를 사토시로 분할하기 때문에",
      "라이트닝 네트워크가 별도의 사토시 토큰을 발행하기 때문에",
    ],
    answer: 1,
    explanation:
  ...
  },
  {
    ...
    options: [
      "채널을 개설할 때 인터넷 연결이 불안정한 문제",
      "송금 방향으로 충분한 잔액이 없어 원하는 경로로 결제를 라우팅하지 못하는 문제",
      "라이트닝 노드가 너무 많아 경로 탐색에 시간이 걸리는 문제",
      "채굴자가 채널 종료 트랜잭션을 처리하지 않는 문제",
    ],
    answer: 1,
    explanation:
...
export default function Ch08LightningNetwork() {
  return (
    <div>
      ...
      <p>
        비트코인 기본 레이어는 의도적으로 보수적으로 설계되어 있습니다.
        약 10분마다 하나의 블록, 제한된 블록 크기 — 이 구조가 탈중앙화와 보안을 가능하게 합니다.
        그러나 전 세계 80억 명이 매일 비트코인으로 거래하려면 온체인만으로는 부족합니다.
      </p>

      <div className="not-prose my-4">
        <KatexBlock
          math={"\\text{On-chain TPS} \\approx \\frac{1{,}000{,}000 \\text{ bytes}}{600 \\text{ s} \\times 250 \\text{ bytes/tx}} \\approx 7 \\text{ TPS}"}
          display={true}
        />
        <p className="text-sm text-muted-foreground text-center mt-1">
          {/* 1,000,000 bytes / (600초 × 250 bytes/tx) ≈ 7 TPS (레거시 바이트 기준) */}
          블록 크기 ÷ (블록 간격 × 평균 트랜잭션 크기) ≈ 7 TPS
        </p>
      </div>

      <InfoBox type="warning" title="블록 공간은 희소 자원">
        Visa는 ~24,000 TPS를 처리합니다. 비트코인 온체인에서 이 수준을 달성하려면 블록 크기를
        수천 배 늘려야 하며, 그렇게 하면 풀노드 운영이 불가능해져 탈중앙화가 무너집니다.
        해결책은 블록체인을 &quot;결산 레이어&quot;로 사용하고, 일상 결제는 레이어2에서 처리하는 것입니다.
      </InfoBox>
