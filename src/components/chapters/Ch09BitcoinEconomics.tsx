...
function AustrianSection() {
  return (
    <section className="mb-10">
      ...
      <p className="mb-3">
        1장에서 건전화폐(Sound Money)의 개념과 미제스, 하이에크의 사상을 소개했습니다.
        이 장에서는 오스트리안 경제학이 비트코인 설계에 어떻게 구체적으로 반영되었는지
        더 깊이 살펴봅니다. 하이에크는 1976년 저서
        <em> The Denationalization of Money</em>에서 &quot;화폐 발행 독점을 국가에서 빼앗아
        시장 경쟁에 맡겨야 한다&quot;고 주장했고, 사토시 나카모토가 2009년 이를 코드로 구현했습니다.
      </p>
...
function TimePreferenceSection() {
  return (
    <section className="mb-10">
      ...
      <p className="mb-3">
        1장에서 간략히 소개한 <strong>시간선호(time preference)</strong> 개념을
        여기서 본격적으로 다룹니다. 시간선호란 미래의 재화보다 현재의 재화를
        얼마나 더 선호하는지를 나타내며, 낮은 시간선호를 가진 사람은
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-muted">
              ...
              <th className="border px-3 py-2 text-right">2025년 가격</th>
  ...
  return (
    <section className="mb-10">
      ...
      <InfoBox type="tip" title="비트코인은 지금 어디?">
        2025년 말 기준 비트코인 보유자는 전 세계 약 4.8억~5억 명으로 추정됩니다.
...
const quizQuestions = [
  ...
  },
  {
    ...
    options: [
      "중앙은행이 인위적으로 이자율을 낮게 유지하기 때문에",
      "정부가 저축세를 부과해 현금 보유를 억제하기 때문에",
      "은행이 계좌 유지 수수료를 부과해 보유를 불리하게 만들기 때문에",
  ...
  },
  {
    ...
    options: [
      ...
      "한 참여자가 독점적으로 이익을 취하고 나머지는 손해를 보는 상태",
      "모든 참여자가 협력해 공동 이익을 극대화하는 안정된 상태",
      "게임이 무한히 반복되면서 참여자들이 협력 전략을 학습하는 상태",
    ],
    answer: 0,
    explanation:
  ...
  },
  {
    ...
    options: [
      "비트코인 가격이 장기적으로 우상향하는 추세를 보이기 때문에",
      "채굴 난이도가 높아져 희소성이 자동으로 보장되기 때문에",
      "국제통화기금(IMF)이 비트코인 준비금 보유를 공식 권고했기 때문에",
  ...
  },
  {
    ...
    options: [
      ...
      "비트코인은 회귀 정리의 예외에 해당하므로 오스트리안 이론 자체를 수정해야 한다",
      "회귀 정리는 실물 상품에만 적용되며 디지털 자산에는 원천적으로 적용되지 않는다",
      "비트코인은 화폐가 아닌 투자 자산이므로 회귀 정리의 적용 대상 자체가 아니다",
    ],
    answer: 0,
    explanation:
