...
function MerkleTreeSim() {
  ...
  const layerLabels = ["트랜잭션 해시 (Leaf)", "레벨 1 (쌍 해시)", "레벨 2", "머클루트"];
  ...
  return (
    <div className="not-prose my-6 rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/20 p-5">
            ...
            {editIdx === i ? (
              <div className="flex gap-1">
                <input
                  ...
                  autoFocus
                />
                <button onClick={commitEdit} className="rounded bg-green-600 text-white px-1.5 text-xs">✓</button>
              </div>
            ) : (
              <button
      ...
      <div className="mt-3 flex justify-end">
        <button onClick={reset} className="text-xs text-muted-foreground hover:text-foreground underline">초기화</button>
      </div>
    </div>
  );
}
